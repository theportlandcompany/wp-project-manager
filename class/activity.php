...
class CPM_Activity {
    ...
    function tasklist_new( $list_id, $project_id ) {
        ...
        $message = sprintf(
            __( 'Task list %s created by %s', 'cpm' ),
    ...
    function tasklist_update( $list_id, $project_id ) {
        $list = get_post( $list_id );
        $message = sprintf(
            __( 'Task list %s updated by %s', 'cpm' ),
            $this->list_url( $list_id, $project_id, $list->post_title ),
            $this->user_url()
        );
    ...
    function tasklist_delete( $list_id ) {
        $list = get_post( $list_id );
        $message = sprintf(
            __( 'Task list "%s" deleted by %s', 'cpm' ),
            $list->post_title,
            $this->user_url()
        );
    ...
    function task_new( $list_id, $task_id ) {
        ...
        $message = sprintf(
            __( 'Task %s added on task list %s by %s', 'cpm' ),
    ...
    function task_undo( $task_id ) {
        ...
        $message = sprintf(
            __( 'Task %s marked un-done by %s', 'cpm' ),
            $this->task_url( $task_id, $list->ID, $list->post_parent, $task->post_title ),
            $this->user_url()
        );

        $task_message = sprintf( __( 'Re-opened task', 'cpm' ) );
    ...
    function task_delete( $task_id ) {
        ...
        $message = sprintf(
            __( 'Task "%s" deleted from task list %s by %s', 'cpm' ),
            $task->post_title,
