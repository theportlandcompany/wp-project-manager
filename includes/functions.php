...
function cpm_task_assign_dropdown( $project_id, $selected = '-1' ) {
    ...
    if ( $users ) {
        ...
        foreach ($users as $user) {
            printf( '<option value="%s"%s>%s</option>', $user['id'], selected( $selected, $user['id'], false ), $user['name'] );
        }
        echo '</select>';
    }
}
function cpm_project_users( $project_id ) {
    $users = CPM_Project::getInstance()->get_users( $project_id );
 ...
 * on messages, tasks and task's comments.
 *
 * @since 0.1
...
function cpm_project_summary( $info ) {
    ...
    if( $info->todolist ) {
        $info_array[] = sprintf( _n( '%d task list', '%d task lists', $info->todolist, 'cpm' ), $info->todolist );
    }

    if( $info->todos ) {
        $info_array[] = sprintf( _n( '%d task', '%d tasks', $info->todos, 'cpm' ), $info->todos );
    }
...
/**
 * Display title on single project view
 *
 * @since 0.3.1.tpc-0.1
 *
...
add_filter( 'admin_title', 'cpm_display_project_title', 10, 2 );