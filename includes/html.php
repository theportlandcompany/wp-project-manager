...
function cpm_task_html( $task, $project_id, $list_id, $single = false ) {
    ...
    <div class="cpm-todo-wrap <?php echo $wrap_class; ?>">
        <span class="cpm-todo-action">
            <a href="#" class="cpm-todo-delete cpm-icon-delete" <?php cpm_data_attr( array('single' => $single, 'list_id' => $list_id, 'project_id' => $project_id, 'task_id' => $task->ID, 'confirm' => __( 'Are you sure to delete this task?', 'cpm' ) ) ); ?>>
                <span><?php _e( 'Delete Task', 'cpm' ); ?></span>
            </a>
        ...
        <span class="cpm-todo-content">
            ...
            if ( $task->completed != '1' ) {
                if ( $task->assigned_to != '-1' ) {
                    ...
                    if ( $user->display_name == $current_user->display_name ) {
                        $highlight_tasks_assigned_to_current_user = 'highlight-task-assignee';
                        $replace_name_with_first_person_verbiage = 'Yours';
                    } else {
                        $replace_name_with_first_person_verbiage = $user->display_name;
                    }
                    ?>
                    <span class="cpm-assigned-user <?php echo $highlight_tasks_assigned_to_current_user; ?>"><?php echo $replace_name_with_first_person_verbiage; ?></span>
                <?php } ?>
            ...
            if ( (int) $task->comment_count > 0 && !$single ) { ?>
                <a class="cpm-comment-count-container" href="<?php echo cpm_url_single_task( $project_id, $list_id, $task->ID ); ?>">
                    <span class="cpm-comment-count-icon"></span>
                    <span class="cpm-comment-count-number"><?php printf( _n( __( '1', 'cpm' ), __( '%d', 'cpm' ), $task->comment_count, 'cpm' ), $task->comment_count ); ?></span>
                </a>
            <?php } ?>
    ...
    <article class="cpm-todolist">
        <header class="cpm-list-header">
            ...
            <table class="cpm-task-lists">
                    ...
                    <td class="task-list-title">
                        ...
                        <h3>
                            ...
                            <div class="cpm-list-actions">
                                <a href="#" class="cpm-list-delete cpm-icon-delete" data-list_id="<?php echo $list->ID; ?>" data-confirm="<?php esc_attr_e( 'Are you sure to delete this task list?', 'cpm' ); ?>">
                ...
                <tr>
                    <td class="cpm-todo-form" colspan="2">
        ...
        <form class="cpm-comment-form">
            ...
            */
            ...
            <div class="notify-users">
                <label class="notify">
                    <?php printf( '<label><a class="cpm-toggle-checkbox" href="#">%s</a></label>', __( 'Select all', 'cpm' ) ); ?>
                </label>
    ...
    <li class="cpm-comment<?php echo $class; ?>" id="cpm-comment-<?php echo $comment->comment_ID; ?>">
        ...
        <div class="cpm-comment-container">
            ...
            <div class="cpm-comment-content">
                ...
                <?php echo $task->ID; ?>
                <?php $strTweet = preg_replace('/(^|\s)#(\w*[a-zA-Z_]+\w*)/', '\1#<a href="http://twitter.com/search?q=%23\2">\2</a>', $strTweet); ?>
...
function cpm_current_tasks( $tasks ) { 
    ...
    <ul id="current-tasks" class="connected">
    <?php foreach ( $tasks as $task ) : ?>
        <?php
        $due_date =  get_post_meta( $task->ID, '_due', true );
        $list_id =  get_post_field( 'post_parent', $task->ID );
